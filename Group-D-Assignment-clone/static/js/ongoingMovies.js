...
function InitializeOngoingMovies() {
  ...
  for (
    let currentIndex = 0;
    currentIndex < moviesArray["movies"].length;
    currentIndex++
  ) {
...
function createGenres() {
  for (
    ...
    currentMovieIndex < moviesArray["movies"].length;
    currentMovieIndex++
  ) {
...
function createFilterProps() {
  ...
  startingOption.classList.add(
    "ongoingMovies__filterDiv__selectDiv__select__option"
  );
  ...
  filterSearchField.classList.add(
    "ongoingMovies__filterDiv__genreChipDiv__searchfield"
  );
